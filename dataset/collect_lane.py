...
def select_mask_points(ct, r, shape, max_sample=5):
    ...
    if len(valid_points) > max_sample:
        valid_points = random.sample(valid_points, max_sample)
    if len(valid_points) < max_sample:
        # Handle the case when there are fewer valid points than required
        # You may choose to add additional points, repeat some points, or handle it as needed.
...
class CollectLane(object):
    ...
    def target(self, results):
        ...
        output_h = self.img_height
        output_w = self.img_width
        ...
        # print(len(gt_hm_lane_ends))
        # print(gt_hm_lane_ends[0])
